...
#cascPath = "cascade_3090p_3019n_15stages_BASIC.xml"
...
cv2.createTrackbar("Alpha", "Result", 50, 1000, callBack)
...
while (cap.isOpened()):
        ...
        if switch_status == 1:
            ...
            if len(faces2) == 0:
                ...
                for (x2, y2, w2, h2) in faces2:
                    ...
                    cv2.putText(img, "IoU: {:.4f}".format(iou), (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        else:
            pass

    cv2.rectangle(img, (10, 10), (10 + 170, 30 + 30), (0, 0, 0), -1)
    ...
    cv2.putText(img, "DPS: {:.4f}".format(fps), (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    cv2.putText(img, "frame: {:.4f}".format(i), (10, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    ...
    if key == ord('q'):
        break
    elif key == ord('f') or i in list_of_screens:
        cv2.imwrite('screenshots/{}.jpg'.format(i), img)
    i = i + 1
cap.release()
